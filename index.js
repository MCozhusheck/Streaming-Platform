    ...
    const worker = cluster
        ...
        .on("message", message => {
            ...
            if (message.action) {
                switch (message.action) {
                    case "index": {
                        let indexer =
                            cluster.workers[_.sample(workers.indexer || [])];
                        if (indexer) {
        ...
        .on("exit", status => {
            ...
            if (worker.exitedAfterDisconnect === true || status === 0) {
                ...
                logger.warn(
                    "Worker %s #%d was died. Replace it with a new one.",
                    name,
                    worker.id
                );
                startWorker(name);
...
function startVODWorkers() {
    if (cluster.isMaster) {
        ...
        const WorkerClass = require(path.join(
            __dirname,
            "workers",
            `${name}.js`
        ));
